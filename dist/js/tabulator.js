...
;(function (global, factory) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (renderInPosition && _this10.getDisplayRows().length) {
				...
				if (_this10.table.options.virtualDomHoz) {
					...
					_this10.table.vdomHoz.deinitialize();
				}
	...
	VDomHoz.prototype.renitializeRows = function () {
		...
		rows.forEach(function (row) {
			...
			_this17.reinitializeRow(row, true);
		});
	};
		...
		if (!this.initialized || force) {
			...
			if (this.table.options.virtualDomHoz && this.table.vdomHoz.initialized) {
				...
				this.cells.forEach(function (cell) {
					...
					_this18.element.appendChild(cell.getElement());
	...
	Row.prototype.updateData = function (updatedData) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (_this19.table.options.reactiveData && _this19.table.modExists("reactiveData", true)) {

				_this19.table.modules.reactiveData.block();
			}

			//mutate incomming data if needed

			if (_this19.table.modExists("mutator")) {

				tempData = Object.assign(tempData, _this19.data);

				tempData = Object.assign(tempData, updatedData);

				newRowData = _this19.table.modules.mutator.transformRow(tempData, "data", updatedData);
			} else {
			...
			for (var attrname in newRowData) {
				...
				_this19.data[attrname] = newRowData[attrname];
			}

			if (_this19.table.options.reactiveData && _this19.table.modExists("reactiveData", true)) {

				_this19.table.modules.reactiveData.unblock();
			}
			...
			for (var attrname in updatedData) {
				...
				var columns = _this19.table.columnManager.getColumnsByFieldRoot(attrname);

				columns.forEach(function (column) {

					var cell = _this19.getCell(column.getField());
			...
			if (visible) {
				...
				_this19.normalizeHeight(true);

				if (_this19.table.options.rowFormatter) {

					_this19.table.options.rowFormatter(_this19.getComponent());
				}
			} else {

				_this19.initialized = false;

				_this19.height = 0;

				_this19.heightStyled = "";
			}

			if (_this19.table.options.dataTree !== false && _this19.table.modExists("dataTree") && _this19.table.modules.dataTree.redrawNeeded(updatedData)) {

				_this19.table.modules.dataTree.initializeRow(_this19);

				_this19.table.modules.dataTree.layoutRow(_this19);

				_this19.table.rowManager.refreshActiveData("tree", false, true);
			}

			//this.reinitialize();


			_this19.table.options.rowUpdated.call(_this19.table, _this19.getComponent());
	...
	Row.prototype.delete = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			if (_this20.table.options.history && _this20.table.modExists("history")) {

				if (_this20.table.options.groupBy && _this20.table.modExists("groupRows")) {

					rows = _this20.getGroup().rows;

					index = rows.indexOf(_this20);
					...
					index = _this20.table.rowManager.getRowIndex(_this20);

					if (index) {

						index = _this20.table.rowManager.rows[index - 1];
					}
				}

				_this20.table.modules.history.action("rowDelete", _this20, { data: _this20.getData(), pos: !index, index: index });
			}

			_this20.deleteActual();
	...
	Row.prototype.deleteCells = function () {
		...
		for (var _i6 = 0; _i6 < cellCount; _i6++) {
	...
	Tabulator.prototype._buildElement = function () {
		...
		if (options.initialHeaderFilter && this.modExists("filter", true)) {
			...
			options.initialHeaderFilter.forEach(function (item) {
				...
				var column = _this21.columnManager.findColumn(item.field);
		...
		return new Promise(function (resolve, reject) {
			...
			input.addEventListener("change", function (e) {
				...
				reader.onload = function (e) {
					...
					_this22.setData(data).then(function (data) {
	...
	Tabulator.prototype.addData = function (data, pos, index) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (data) {
				...
				_this24.rowManager.addRows(data, pos, index).then(function (rows) {
	...
	Tabulator.prototype.deleteRow = function (index) {
		...
		return new Promise(function (resolve, reject) {
			...
			index.forEach(function (item) {
				...
				var row = _this26.rowManager.findRow(item, true);
			...
			foundRows.sort(function (a, b) {
				...
				return _this26.rowManager.rows.indexOf(a) > _this26.rowManager.rows.indexOf(b) ? 1 : -1;
			});
	...
	Tabulator.prototype.addRow = function (data, pos, index) {
		...
		return new Promise(function (resolve, reject) {
			...
			_this27.rowManager.addRows(data, pos, index).then(function (rows) {
				...
				if (_this27.modExists("columnCalcs")) {

					_this27.modules.columnCalcs.recalc(_this27.rowManager.activeRows);
				}
	...
	Tabulator.prototype.updateOrAddRow = function (index, data) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (row) {
				...
				row = _this28.rowManager.addRows(data).then(function (rows) {
	...
	Tabulator.prototype.scrollToRow = function (index, position, ifVisible) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (row) {
				...
				_this30.rowManager.scrollToRow(row, position, ifVisible).then(function () {
	...
	Tabulator.prototype.addColumn = function (definition, before, field) {
		...
		return new Promise(function (resolve, reject) {
			...
			_this31.columnManager.addColumn(definition, before, column).then(function (column) {
	...
	Tabulator.prototype.scrollToColumn = function (field, position, ifVisible) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (column) {
				...
				_this34.columnManager.scrollToColumn(column, position, ifVisible).then(function () {
	...
	Tabulator.prototype.clearCellValidation = function (cells) {
		...
		if (this.modExists("validate", true)) {
			...
			cells.forEach(function (cell) {
				...
				_this35.modules.validate.clearValidation(cell._getSelf());
			});
		}
	};
	...
	Tabulator.prototype.setPageToRow = function (row) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (_this36.options.pagination && _this36.modExists("page")) {

				row = _this36.rowManager.findRow(row);

				if (row) {

					_this36.modules.page.setPageToRow(row).then(function () {
	...
	Tabulator.prototype.clearCellEdited = function (cells) {
		...
		if (this.modExists("edit", true)) {
			...
			cells.forEach(function (cell) {
				...
				_this37.modules.edit.clearEdited(cell._getSelf());
			});
		}
	};
	...
	Layout.prototype.modes = {
		...
		"fitDataStretch": function fitDataStretch(columns) {
			...
			columns.forEach(function (column, i) {
				...
				if (_this38.table.options.responsiveLayout ? column.modules.responsive.visible : column.visible) {
	...
	Ajax.prototype._loadDataStandard = function (inPosition, columnsChanged) {
		...
		return new Promise(function (resolve, reject) {
			_this39.sendRequest(inPosition).then(function (data) {
				_this39.table.rowManager.setData(data, inPosition, columnsChanged).then(function () {
					resolve();
				}).catch(function (e) {
					reject(e);
				});
	...
	Ajax.prototype.sendRequest = function (silent) {
		...
		return new Promise(function (resolve, reject) {
			if (self.table.options.ajaxRequesting.call(_this40.table, self.url, self.params) !== false) {
				...
				_this40.loaderPromise(url, self.config, self.params).then(function (data) {
	...
	ColumnCalcs.prototype.rowsToData = function (rows) {
		...
		rows.forEach(function (row) {
			...
			if (_this42.table.options.dataTree && _this42.table.options.dataTreeChildColumnCalcs) {
				if (row.modules.dataTree.open) {
					var children = _this42.rowsToData(_this42.table.modules.dataTree.getFilteredTreeChildren(row));
		...
		if (this.mode === true || this.mode === "copy") {
			this.table.element.addEventListener("copy", function (e) {
				...
				if (!_this43.blocked) {
					...
					if (_this43.customSelection) {
						plain = _this43.customSelection;

						if (_this43.table.options.clipboardCopyFormatter) {
							plain = _this43.table.options.clipboardCopyFormatter("plain", plain);
						}
					} else {

						var list = _this43.table.modules.export.generateExportList(_this43.rowRange, _this43.table.options.clipboardCopyStyled, _this43.table.options.clipboardCopyConfig, "clipboard");

						html = _this43.table.modules.export.genereateHTMLTable(list);
						plain = html ? _this43.generatePlainContent(list) : "";

						if (_this43.table.options.clipboardCopyFormatter) {
							plain = _this43.table.options.clipboardCopyFormatter("plain", plain);
							html = _this43.table.options.clipboardCopyFormatter("html", html);
						}
					}
					...
					_this43.table.options.clipboardCopied.call(_this43.table, plain, html);
		...
		if (config.children !== false) {
			...
			if (config.open) {
				...
				config.controlEl.addEventListener("click", function (e) {
					e.stopPropagation();
					_this44.collapseRow(row);
				});
			} else {
				config.controlEl = this.expandEl.cloneNode(true);
				config.controlEl.addEventListener("click", function (e) {
					e.stopPropagation();
					_this44.expandRow(row);
				});
			}
		...
		rows.forEach(function (row, i) {
			...
			if (row instanceof Row) {
				...
				if (!config.index && config.children !== false) {
					children = _this45.getChildren(row);
	...
	DataTree.prototype.getChildren = function (row) {
		...
		if (config.children !== false && config.open) {
			...
			children.forEach(function (child) {
				...
				var subChildren = _this46.getChildren(child);
	...
	DataTree.prototype.generateChildren = function (row) {
		...
		childArray.forEach(function (childData) {
			var childRow = new Row(childData || {}, _this47.table.rowManager);
			childRow.modules.dataTree.index = row.modules.dataTree.index + 1;
			childRow.modules.dataTree.parent = row;
			if (childRow.modules.dataTree.children) {
				childRow.modules.dataTree.open = _this47.startOpen(childRow.getComponent(), childRow.modules.dataTree.index);
			}
	...
	DataTree.prototype.findChildIndex = function (subject, parent) {
		...
		if ((typeof subject === 'undefined' ? 'undefined' : _typeof(subject)) == "object") {
			...
			match = parent.data[this.field].find(function (row) {
				return row.data[_this48.table.options.index] == subject;
			});
		}
	...
	Edit.prototype.editors = {
		...
		select: function select(cell, onRendered, success, cancel, editorParams) {
			...
			setTimeout(function () {
				_this49.table.rowManager.element.addEventListener("scroll", cancelItem);
			}, 10);
		...
		autocomplete: function autocomplete(cell, onRendered, success, cancel, editorParams) {
			...
			setTimeout(function () {
				_this50.table.rowManager.element.addEventListener("scroll", cancelItem);
			}, 10);
	...
	Export.prototype.rowLookup = function (range) {
		...
		if (typeof range == "function") {
			range.call(this.table).forEach(function (row) {
				row = _this51.table.rowManager.findRow(row);
	...
	Export.prototype.generateColumnGroupHeaders = function () {
		...
		columns.forEach(function (column) {
			var colData = _this52.processColumnGroup(column);
	...
	Export.prototype.processColumnGroup = function (column) {
		...
		if (subGroups.length) {
			...
			subGroups.forEach(function (subGroup) {
				var subGroupData = _this53.processColumnGroup(subGroup);
	...
	Export.prototype.headersToExportRows = function (columns) {
		...
		function parseColumnGroup(column, level) {
			...
			if (column.height > 1) {
				for (var _i8 = 1; _i8 < column.height; _i8++) {

					if (typeof headers[level + _i8] === "undefined") {
						headers[level + _i8] = [];
					}

					headers[level + _i8].push(false);
				}
			}

			if (column.width > 1) {
				for (var _i9 = 1; _i9 < column.width; _i9++) {
	...
	Export.prototype.bodyToExportRows = function (rows) {
		...
		this.table.columnManager.columnsByIndex.forEach(function (column) {
			if (_this54.columnVisCheck(column)) {
		...
		rows = rows.filter(function (row) {
			switch (row.type) {
				case "group":
					return _this54.config.rowGroups !== false;
					break;

				case "calc":
					return _this54.config.columnCalcs !== false;
					break;

				case "row":
					return !(_this54.table.options.dataTree && _this54.config.dataTree === false && row.modules.dataTree.parent);
					break;
			}
		...
		rows.forEach(function (row, i) {
			var rowData = row.getData(_this54.colVisProp);
			var exportCols = [];
			var indent = 0;
			...
			switch (row.type) {
				...
				case "row":
					...
					if (_this54.table.options.dataTree && _this54.config.dataTree !== false) {
	...
	Export.prototype.genereateTableElement = function (list) {
		...
		list.forEach(function (row, i) {
			switch (row.type) {
				case "header":
					headerEl.appendChild(_this55.genereateHeaderElement(row, setup, styles));
					break;

				case "group":
					bodyEl.appendChild(_this55.genereateGroupElement(row, setup, styles));
					break;

				case "calc":
					bodyEl.appendChild(_this55.genereateCalcElement(row, setup, styles));
					break;

				case "row":
					var rowEl = _this55.genereateRowElement(row, setup, styles);
					_this55.mapElementStyles(i % 2 && styles.evenRow ? styles.evenRow : styles.oddRow, rowEl, ["border-top", "border-left", "border-right", "border-bottom", "color", "font-weight", "font-family", "font-size", "background-color"]);
	...
	Export.prototype.genereateHeaderElement = function (row, setup, styles) {
		...
		row.columns.forEach(function (column) {
			if (column) {
				...
				_this56.mapElementStyles(column.component.getElement(), cellEl, ["text-align", "border-top", "border-left", "border-right", "border-bottom", "background-color", "color", "font-weight", "font-family", "font-size"]);
				_this56.mapElementStyles(column.component._column.contentElement, cellEl, ["padding-top", "padding-left", "padding-right", "padding-bottom"]);

				if (column.component._column.visible) {
					_this56.mapElementStyles(column.component.getElement(), cellEl, ["width"]);
				} else {
				...
				if (column.component._column.parent) {
					_this56.mapElementStyles(column.component._column.parent.groupElement, cellEl, ["border-top"]);
				}
	...
	Export.prototype.genereateRowElement = function (row, setup, styles) {
		...
		row.columns.forEach(function (col) {
			...
			if (col) {
				...
				if (_this57.table.modExists("format") && _this57.config.formatCells !== false) {
					value = _this57.table.modules.format.formatExportValue(cellWrapper, _this57.colVisProp);
				} else {
					switch (typeof value === 'undefined' ? 'undefined' : _typeof(value)) {
						case "object":
				...
				if (styles.firstCell) {
					_this57.mapElementStyles(styles.firstCell, cellEl, ["padding-top", "padding-left", "padding-right", "padding-bottom", "border-top", "border-left", "border-right", "border-bottom", "color", "font-weight", "font-family", "font-size"]);
				...
				if (setup.rowFormatter && _this57.config.formatCells !== false) {
		...
		if (field) {
			...
			if (editor) {
				...
				editorElement.addEventListener("focus", function (e) {
					var left = _this58.table.columnManager.element.scrollLeft;

					if (left !== _this58.table.rowManager.element.scrollLeft) {
						_this58.table.rowManager.scrollHorizontal(left);
						_this58.table.columnManager.scrollHorizontal(left);
					}
				});
	...
	Filter.prototype.filtersToArray = function (filterList, ajax) {
		...
		filterList.forEach(function (filter) {
			...
			if (Array.isArray(filter)) {
				output.push(_this59.filtersToArray(filter, ajax));
			} else {
		...
		rowSelection: function rowSelection(cell, formatterParams, onRendered) {
			...
			if (this.table.modExists("selectRow", true)) {
				...
				if (typeof cell.getRow == 'function') {
					...
					checkbox.addEventListener("change", function (e) {
						if (_this60.table.modules.selectRow.selectedRows.length) {
							_this60.table.deselectRow();
						} else {
							_this60.table.selectRow(formatterParams.rowRange);
						}
					});
	...
	FrozenColumns.prototype.layoutColumnPosition = function (allCells) {
		...
		this.leftColumns.forEach(function (column, i) {
			column.modules.frozen.margin = _this62._calcSpace(_this62.leftColumns, i) + _this62.table.columnManager.scrollLeft + "px";

			if (i == _this62.leftColumns.length - 1) {
			...
			if (column.parent.isGroup) {
				var parentEl = _this62.getColGroupParentElement(column);
				if (!leftParents.includes(parentEl)) {
					_this62.layoutElement(parentEl, column);
				...
				_this62.layoutElement(column.getElement(), column);
			}

			if (allCells) {
				column.cells.forEach(function (cell) {
					_this62.layoutElement(cell.getElement(), column);
				});
			}
		});

		this.rightColumns.forEach(function (column, i) {
			column.modules.frozen.margin = _this62.rightPadding - _this62._calcSpace(_this62.rightColumns, i + 1) + "px";

			if (i == _this62.rightColumns.length - 1) {
				column.modules.frozen.edge = true;
			} else {
				column.modules.frozen.edge = false;
			}
			...
			if (column.parent.isGroup) {
				_this62.layoutElement(_this62.getColGroupParentElement(column), column);
			} else {
				_this62.layoutElement(column.getElement(), column);
			}
	...
	FrozenColumns.prototype.layoutRow = function (row) {
		...
		this.leftColumns.forEach(function (column) {
			...
			if (cell) {
				_this63.layoutElement(cell.getElement(), column);
			}
		});
	...
	Group.prototype.createValueGroups = function () {
		...
		if (this.groupManager.allowedValues && this.groupManager.allowedValues[level]) {
			this.groupManager.allowedValues[level].forEach(function (value) {
				_this64._createGroup(value, level);
			});
		}
	};
	...
	GroupRows.prototype.getChildGroups = function (group) {
		...
		group.groupList.forEach(function (child) {
			if (child.groupList.length) {
				groupComponents = groupComponents.concat(_this65.getChildGroups(child));
			} else {
	...
	Menu.prototype.initializeColumnHeader = function (column) {
		...
		if (column.definition.headerContextMenu) {
			column.getElement().addEventListener("contextmenu", function (e) {
				...
				_this67.loadMenu(e, column, menu);
			});
		}
	...
	Menu.prototype.initializeRow = function (row) {
		...
		row.getElement().addEventListener("contextmenu", function (e) {
			var menu = typeof _this69.table.options.rowContextMenu == "function" ? _this69.table.options.rowContextMenu(row.getComponent()) : _this69.table.options.rowContextMenu;

			_this69.loadMenu(e, row, menu);
		});
	};
	...
	Menu.prototype.initializeGroup = function (group) {
		...
		group.getElement().addEventListener("contextmenu", function (e) {
			var menu = typeof _this70.table.options.groupContextMenu == "function" ? _this70.table.options.groupContextMenu(group.getComponent()) : _this70.table.options.groupContextMenu;

			_this70.loadMenu(e, group, menu);
		});
	};
	...
	ResponsiveLayout.prototype.initialize = function () {
		...
		for (var _iterator = this.table.columnManager.columnsByIndex, _isArray = Array.isArray(_iterator), _i14 = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
			var _ref;
	...
	SelectRow.prototype.deselectRows = function (rows, silent) {
		...
		if (typeof rows == "undefined") {
			...
			for (var _i15 = 0; _i15 < rowCount; _i15++) {
	...
	SelectRow.prototype.childRowSelection = function (row, select) {
		...
		if (select) {
			for (var _iterator2 = children, _isArray2 = Array.isArray(_iterator2), _i16 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
				var _ref2;
			...
			for (var _iterator3 = children, _isArray3 = Array.isArray(_iterator3), _i17 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;) {
				var _ref3;
