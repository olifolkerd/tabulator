	...
	constructor(def, parent){
		...
		if(def.columns){
			...
			def.columns.forEach(function(def, i){
				var newCol = new Column$1(def, self);
	...
	generateRow(pos, data){
		...
		row.generateCells = function(){
			...
			self.table.columnManager.columnsByIndex.forEach(function(column){
					...
					var cell = new Cell$1(self.genColumn, row);
					cell.getElement();
					cell.column = column;
					cell.setWidth();
...
class Tabulator$1 {
	...
	constructor(element, options){
		...
		Tabulator$1.comms.register(this); //register table for inderdevice communication
	}

	initializeOptions(options){

		var defaults = Tabulator$1.defaultOptions;
	...
	bindModules(){
		...
		for(var name in Tabulator$1.moduleBindings){
			this.modules[name] = new Tabulator$1.moduleBindings[name](this);
		}
	}

	//extend module
	static extendModule = function(name, property, values){
		if(Tabulator$1.moduleBindings[name]){
			var source = Tabulator$1.moduleBindings[name].prototype[property];
	...
	static registerModuleImport(modules){
		...
		Tabulator$1.registerModule(mods);
	}
	...
	static registerModule(modules){
		...
		modules.forEach((mod) => {
			Tabulator$1.registerModuleBinding(mod);
		});
	}

	//add module to tabulator
	static registerModuleBinding(mod){
		Tabulator$1.moduleBindings[mod.moduleName] = mod;
	};
	...
	static comms = {
		...
		register:function(table){
			Tabulator$1.comms.tables.push(table);
		},
		deregister:function(table){
			var index = Tabulator$1.comms.tables.indexOf(table);

			if(index > -1){
				Tabulator$1.comms.tables.splice(index, 1);
			}
		},
			...
			if(typeof query === "string"){
				...
				if(matches.length){
					for(var i = 0; i < matches.length; i++){
						match = Tabulator$1.comms.matchElement(matches[i]);
				...
				match = Tabulator$1.comms.matchElement(query);
				...
				query.forEach(function(item){
					results = results.concat(Tabulator$1.comms.lookupTable(item));
				});
			}else {
				if(!silent){
		...
		matchElement:function(element){
			return Tabulator$1.comms.tables.find(function(table){
	...
	static findTable(query){
		var results = Tabulator$1.comms.lookupTable(query, true);
		return Array.isArray(results) && !results.length ? false : results;
	}
}

Tabulator$1.registerModuleImport(coreModules);

Tabulator$1.registerModuleImport(modules);

export { Accessor as AccessorModule, Ajax as AjaxModule, Clipboard as ClipboardModule, ColumnCalcs as ColumnCalcsModule, DataTree as DataTreeModule, Download as DownloadModule, Edit as EditModule, Export as ExportModule, Filter as FilterModule, Format as FormatModule, FrozenColumns as FrozenColumnsModule, FrozenRows as FrozenRowsModule, GroupRows as GroupRowsModule, History as HistoryModule, HtmlTableImport as HtmlTableImportModule, Keybindings as KeybindingsModule, Menu as MenuModule, MoveColumns as MoveColumnsModule, MoveRows as MoveRowsModule, Mutator as MutatorModule, Page as PageModule, Persistence as PersistenceModule, Print as PrintModule, ReactiveData as ReactiveDataModule, ResizeColumns as ResizeColumnsModule, ResizeRows as ResizeRowsModule, ResizeTable as ResizeTableModule, ResponsiveLayout as ResponsiveLayoutModule, SelectRow as SelectRowModule, Sort as SortModule, Tabulator$1 as Tabulator, Tabulator$1 as TabulatorCore, Validate as ValidateModule };
